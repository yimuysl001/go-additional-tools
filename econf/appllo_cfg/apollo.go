...
type Client struct {
	...
	value      *gjson.Json           // Configmap content cached. It is `*gjson.Json` value internally.
	watchers   *gcfg.WatcherRegistry // Watchers for watching file changes.
	localValue *gjson.Json
}

// New creates and returns gcfg.Adapter implementing using apollo service.
func New(ctx context.Context, config Config, localvue *gjson.Json) (adapter gcfg.Adapter, err error) {
	// Data validation.
	...
	if config.NamespaceName == "" {
		config.NamespaceName = "application.properties" //storage.GetDefaultNamespace()
	}
	client := &Client{
		config:     config,
		localValue: localvue,
		value:      gjson.New(nil, true),
...
func (c *Client) Get(ctx context.Context, pattern string) (value any, err error) {
	...
	if val := c.value.Get(pattern).Val(); val != nil {
		return val, nil
	}
	return c.localValue.Get(pattern).Val(), nil
}
